...
function addToXml(form, category) {
    switch (category) {
        case "rooms":
            ...
            xmldata.push(pushStuff("Nto", fillEmpty("selN", "0")));
            xmldata.push(pushStuff("Sto", fillEmpty("selS", "0")));
            xmldata.push(pushStuff("Eto", fillEmpty("selE", "0")));
            xmldata.push(pushStuff("Wto", fillEmpty("selW", "0")));
            xmldata.push(pushStuff("Upto", fillEmpty("selUP", "0")));
            xmldata.push(pushStuff("Dto", fillEmpty("selD", "0")));
            xmldata.push(pushStuff("Into", fillEmpty("selIN", "0")));
            xmldata.push(pushStuff("Outto", fillEmpty("selOUT", "0")));
...
function emptyTalk() {
    ...
    for (x = 1; x <= 5; x++) {
        ...
        fetchPeepsNames(obNm, "No person " + x, "x");
        fillEmpty(obNm, "x");
...
var deleteThings = function (frm, cat) {
    switch (cat) {
        case "room":
            if (confirm("Are you sure you want to delete this room?") == true) {
                document.getElementById("RmName").value = "xx";
                download(frm, "rooms");
            } else {
                return false;
            };
        ...
        case "objects":
            if (confirm("Are you sure you want to delete this object?") == true) {
                document.getElementById("ObName").value = "xx";
                download(frm, "objects");
            } else {
                return false;
            };
            ...
            break;
        case "peeps":
            if (confirm("Are you sure you want to delete this NPC?") == true) {
                document.getElementById("pName").value = "xx";
                download(frm, "peeps");
            } else {
                return false;
            };
        ...
        case "talk":
            if (confirm("Are you sure you want to delete this conversation?") == true) {
                document.getElementById("line1").value = "x";
                download(frm, "talk");
            } else {
                return false;
            };
...
var autoBuild = function () {
    // try to build game from inside app. either by forcing terminal window and commands or by editing this:
    /*
    "use strict"
